/// @file fstack_busy_poll_example.cpp
/// @brief F-Stack DPDK user-space TCP/IP stack friendly multi-threaded busy-polling example.
...
int main(int argc, char* argv[]) {
    ...
    std::jthread logger_thread([&](std::stop_token stoken) {
        ...
        while (g_running && !stoken.stop_requested()) {
            ...
            while (TscClock::tscns() - start_sleep < 5000) {
    ...
    // 5b. Spawn Core Thread (Collects Tick and Latency Statistics)
    ...
    std::thread core_thread([&]() {
        // Pin strategy core thread to CPU Core 2 (F-Stack runs on main thread core)
